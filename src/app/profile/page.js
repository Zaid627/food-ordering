...
import toast from "react-hot-toast";
import UserTabs from "../../components/layout/UserTabs";
import UserForm from "../../components/layout/UserFrom"; // Corrected name from UserFrom to UserForm
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (status === "authenticated") {
      // Fetch the profile data from /api/profile
      fetch("/api/profile")
        .then(async (res) => {
          if (!res.ok) {
            const text = await res.text();
        ...
        .then((data) => {
          setUser(data); // Save the fetched profile data
          setIsAdmin(data.admin || false); // Set admin status
          setProfileFetched(true);
        })
        .catch((err) => {
          console.error("Error loading profile:", err);
